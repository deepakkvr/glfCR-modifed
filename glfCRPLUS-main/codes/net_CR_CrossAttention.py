...
from net_swin_parts import SwinTransformerBlock, PatchMerging, PatchEmbed

class SwinEncoder(nn.Module):
    """
    Hierarchical Swin Transformer Encoder
    Produces multi-scale features: E1 (H,W), E2 (H/2,W/2), E3 (H/4,W/4)
    """
    def __init__(self, in_chans=13, embed_dim=64, depths=[2, 2, 2], num_heads=[2, 4, 8],
        ...
        # Patch Embedding: (B, C, H, W) -> (B, Ph*Pw, EmbedDim)
        # Note: We want to keep resolution H,W for E1 roughly similar to input or handle downsampling carefully.
        # Standard Swin starts with 4x downsampling. Here we want E1 at input resolution or similar for UNet structure.
        # But `OpticalEncoder` E1 was at H,W. `ConvBlock(kernel=3, stride=1)` maintains resolution.
        # To maintain H,W at E1, we use patch_size=1 (linear projection per pixel).
        self.patch_embed = PatchEmbed(
            img_size=256, patch_size=1, in_chans=in_chans, embed_dim=embed_dim
        )
        ...
        for i_layer in range(self.num_layers):
            ...
            input_resolution = (256 // (2 ** i_layer), 256 // (2 ** i_layer)) # Assumption: input is 256
            ...
            # Patch Merging (Downsampling) - placed AFTER the blocks for the NEXT stage
            # Use module list to hold blocks
            layer = nn.ModuleDict({
                'blocks': nn.Sequential(*blocks),
                'downsample': PatchMerging(input_resolution, dim) if i_layer < self.num_layers - 1 else nn.Identity()
            })
    ...
    def forward(self, x):
        ...
        outs = []
        B, L, C = x.shape
        H = W = int(np.sqrt(L)) # Assuming square
        
        # Stage 1
        x = self.layers[0]['blocks'](x) # (B, H*W, C)
        x_reshaped = x.view(B, H, W, C).permute(0, 3, 1, 2) # (B, C, H, W)
        outs.append(x_reshaped)
        
        x = self.layers[0]['downsample'](x) # (B, H/2*W/2, 2C)
        
        # Stage 2
        x = self.layers[1]['blocks'](x)
        H, W = H//2, W//2
        x_reshaped = x.view(B, H, W, int(C*2)).permute(0, 3, 1, 2)
        outs.append(x_reshaped)
        
        x = self.layers[1]['downsample'](x)
        
        # Stage 3
        x = self.layers[2]['blocks'](x)
        H, W = H//2, W//2
        x_reshaped = x.view(B, H, W, int(C*4)).permute(0, 3, 1, 2)
        outs.append(x_reshaped)
...
class CloudRemovalCrossAttention(nn.Module):
    ...
    def __init__(self, num_heads=8, qkv_bias=True, qk_scale=None, 
        ...
        # Dual encoders - Swin Transformer Backbone
        self.optical_encoder = SwinEncoder(in_chans=13)
        self.sar_encoder = SwinEncoder(in_chans=2)
