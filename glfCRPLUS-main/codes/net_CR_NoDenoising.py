"""
Cloud Removal Network with Cross-Attention (NO DENOISING ABLATION)
Architecture: Dual Encoders (Optical + SAR) -> Cross-Attention Fusion -> Refinement (No Gating)

ABLATION VARIANT: Removes speckle-aware gating to test the effect of no SAR noise handling.
...
class CloudRemovalCrossAttentionNoDenoising(nn.Module):
    """
    ABLATION VARIANT: No SAR Noise Handling
    
    Complete Cloud Removal Network with Cross-Attention (NO Speckle-Aware Gating)
    
    Architecture:
    1. Dual Encoders: Separate optical (13ch) and SAR (2ch) encoders
    2. Global Cross-Attention: SAR guides optical feature refinement
    3. NO GATING: Cross-attention output passed directly to refinement
    4. Refinement: Fuses cross-attention output with original optical
    5. Symmetric Decoder: Upsampling with skip connections, outputs 13 channels
    
    Difference from Original: Speckle-aware gating module is REMOVED.
    Cross-attention output goes directly to refinement block.
    """
    
    def __init__(self, num_heads=8, qkv_bias=True, qk_scale=None, 
                 attn_drop=0., proj_drop=0.):
        super(CloudRemovalCrossAttentionNoDenoising, self).__init__()
    ...
    def forward(self, optical_img, sar_img):
        ...
        # Here we simply use cross_out directly
        gated_out = cross_out  # Direct passthrough, no noise handling
...
def create_model(pretrained=False, **kwargs):
    """
    Create CloudRemovalCrossAttentionNoDenoising model
    Args:
        pretrained: bool - whether to load pretrained weights (not implemented)
        **kwargs: additional arguments for model initialization
    Returns:
        model: CloudRemovalCrossAttentionNoDenoising instance
    """
    model = CloudRemovalCrossAttentionNoDenoising(**kwargs)
    return model
...
if __name__ == "__main__":
    ...
    model = CloudRemovalCrossAttentionNoDenoising()
    ...
    print("\n✓ NoDenoising variant test successful!")
