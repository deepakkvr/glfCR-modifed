"""
Cloud Removal Network with Cross-Attention (FULL DENOISING ABLATION)
Architecture: Dual Encoders (Optical + SAR) -> SAR Denoiser -> Cross-Attention Fusion -> Refinement

ABLATION VARIANT: Adds explicit SAR denoising before cross-attention.
Removes speckle-aware gating and uses upfront denoising instead.
All other components remain identical to the original model for fair comparison.
"""
...
class SARDenoiser(nn.Module):
    """
    Lightweight SAR denoising module applied BEFORE cross-attention.
    Uses simple convolutional layers to suppress speckle noise.
    """
    def __init__(self, dim=256):
        super(SARDenoiser, self).__init__()
    ...
    def forward(self, feat_sar):
        """
        Args:
            feat_sar: (B, C, H, W) - noisy SAR features
        Returns:
            denoised_sar: (B, C, H, W) - denoised SAR features
        """
        # Apply denoising network
        denoised = self.denoiser(feat_sar)
        # Residual connection for stability
        denoised_sar = feat_sar + denoised
        return denoised_sar
...
class Refinement(nn.Module):
    """Refinement block: fuses cross-attention output with original optical features"""
    def __init__(self, dim=256):
    ...
    def forward(self, cross_out, feat_opt_3):
        """
        Args:
            cross_out: (B, C, H, W) - cross-attention output (NOT gated in this variant)
            feat_opt_3: (B, C, H, W) - original optical E3 features
        Returns:
            refined: (B, C, H, W) - refined features
        """
        # Concatenate and fuse
        concat = torch.cat([cross_out, feat_opt_3], dim=1)  # (B, 2C, H, W)
...
class CloudRemovalCrossAttentionFullDenoising(nn.Module):
    """
    ABLATION VARIANT: Full SAR Denoising
    
    Complete Cloud Removal Network with Explicit SAR Denoising
    
    Architecture:
    1. Dual Encoders: Separate optical (13ch) and SAR (2ch) encoders
    2. SAR Denoiser: Explicit denoising of SAR features BEFORE cross-attention
    3. Global Cross-Attention: Denoised SAR guides optical feature refinement
    4. NO GATING: Cross-attention output passed directly to refinement
    5. Refinement: Fuses cross-attention output with original optical
    6. Symmetric Decoder: Upsampling with skip connections, outputs 13 channels
    
    Difference from Original: Speckle-aware gating is REPLACED with upfront SAR denoising.
    SAR features are denoised before entering cross-attention.
    """
    
    def __init__(self, num_heads=8, qkv_bias=True, qk_scale=None, 
                 attn_drop=0., proj_drop=0.):
        super(CloudRemovalCrossAttentionFullDenoising, self).__init__()
        
        # Dual encoders
        self.optical_encoder = OpticalEncoder()
        self.sar_encoder = SAREncoder()
        
        # SAR Denoiser (NEW: replaces speckle-aware gating)
        self.sar_denoiser = SARDenoiser(dim=256)
        ...
        # NOTE: SpeckleAwareGatingModule is REMOVED for this ablation variant
    ...
    def forward(self, optical_img, sar_img):
        ...
        # ABLATION: Apply explicit SAR denoising BEFORE cross-attention
        feat_sar_3_denoised = self.sar_denoiser(feat_sar_3)  # (B, 256, H/4, W/4)
        
        # Global cross-attention fusion with DENOISED SAR features
        cross_out = self.cross_attn(feat_opt_3, feat_sar_3_denoised)  # (B, 256, H/4, W/4)
        
        # ABLATION: NO GATING - pass cross_out directly to refinement
        # In the original model: gated_out = self.speckle_gating(cross_out, feat_sar_3)
        # Here we simply use cross_out directly (already using denoised SAR)
        
        # Refinement: fuse with original optical features
        refined_3 = self.refinement(cross_out, feat_opt_3)  # (B, 256, H/4, W/4)
...
def create_model(pretrained=False, **kwargs):
    """
    Create CloudRemovalCrossAttentionFullDenoising model
    Args:
        pretrained: bool - whether to load pretrained weights (not implemented)
        **kwargs: additional arguments for model initialization
    Returns:
        model: CloudRemovalCrossAttentionFullDenoising instance
    """
    model = CloudRemovalCrossAttentionFullDenoising(**kwargs)
    return model
...
if __name__ == "__main__":
    ...
    model = CloudRemovalCrossAttentionFullDenoising()
    ...
    print("\n✓ FullDenoising variant test successful!")
